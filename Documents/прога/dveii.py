...
class Piece():
    def minimax_W(self, position, depth, max_player, board):
        ...
        if (max_player == 'w'):
            ...
            best_move = None
            i = 0
            for move in get_all_moves('b', board, position[i], 'w'):
                tmp = position[i]
                i += 1
                evaluation = y.minimax_W(moves, depth-1, False, board)
            ...
            best_move = None
            j = 0
            for move in get_all_moves('w', board, position[j], 'b'):
                j += 1
    ...
    def minimax_B(self, position, depth, max_player, board):
        ...
        if (max_player == 'b'):
            ...
            best_move = None
            i = 0
            for move in get_all_moves('w', board, position[i], 'b'):
                tmp = position[i]
                i += 1
                evaluation = y.minimax_B(moves, depth-1, False, board)
                print('minimacs прошел дальше')
                if maxEval < evaluation:
                    best_move = tmp
                print(best_move, 'из минимакса перед возвращением')
            ...
            best_move = None
            j = 0
            for move in get_all_moves('b', board, position[j], 'w'):
                j += 1
                evaluation = y.minimax_B(moves, depth-1, True, board)
                minEval = min(minEval, evaluation)
            return minEval
def simulate_move(move, board, color1):
    chet, x1, y1, x2, y2 = move
    x = Shashki()
    board = x.change(board, chet, color1, x1, y1, x2, y2)
    return board
def get_all_moves(color, board, move, cl):
    moves = []
    x = Shashki()
    board = simulate_move(move, board, cl)#белый походил, доска изменилась
    moves = x.getNextTurn(board, color)
    return moves
...
while (proverka == 0):
    ...
    if motion == 'w':
            print(moves, 'w')
            max_player = 'w'
            best_move = y.minimax_W(moves, 3, max_player, m)
     ...
     #       max_player = 'b'
      #      best_move = minimax_B(moves, 1, max_player, m)
       #     m = new_board
            ...
            print(moves, 'b')
            max_player = 'b'
            best_move = y.minimax_B(moves, 2, max_player, m)
            if isinstance(best_move, float) == 0:
                chet, x1, y1, x2, y2 = best_move
            else:
                chet, x1, y1, x2, y2 = -1, 0, 0, 0, 0
